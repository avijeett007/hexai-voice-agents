...
async def prewarm(proc: JobProcess):
    """Prewarm the process by loading models"""
    try:
        # Load VAD model only
...
async def entrypoint(ctx: JobContext):
    ...
    # Register exit handler early in the process
    ...
    if metadata:
        try:
            ...
            user_data = UserData(metadata_json.get('email', ''), agent_type)
...
async def setup_disc_agent(ctx: JobContext, participant: rtc.Participant, user_data: UserData):
    ...
    logger.info("Setting up DISC Assessment agent")
    
    # Initialize DISC agent
    disc_agent = DISCAssessmentAgent(user_data)
    
    # Register in global tracking for signal handlers
    active_agents['disc'] = disc_agent
    ...
    async def end_of_session():
        ...
        # Mark shutdown callbacks as run
        shutdown_callbacks_run = True
        logger.info("Marked shutdown callbacks as run")
    
    # Add to shutdown callbacks - KEY DIFFERENCE: Using ctx directly
    ctx.add_shutdown_callback(end_of_session)
    
    # Wait for disconnection - keep this so function doesn't exit prematurely
    try:
        # Monitor connection state
        while ctx.room.connection_state == rtc.ConnectionState.CONN_CONNECTED:
            await asyncio.sleep(1)
        
        logger.info("Room disconnected, waiting for shutdown callbacks to run")
    except Exception as e:
...
async def setup_coach_agent(ctx: JobContext, participant: rtc.Participant, user_data: UserData):
    ...
    logger.info("Setting up professional coach agent")
    ...
    # Register in global tracking for signal handlers
    active_agents['coach'] = coach_agent
...
# Global variable to track if exit processing has been done
exit_processing_done = False
shutdown_callbacks_run = False

# Register an atexit handler to ensure end-of-session processing
def register_exit_handler():
    """Register an exit handler to ensure end-of-session processing"""
    
    def exit_handler():
        """Run end-of-session processing before the process exits"""
        global exit_processing_done, shutdown_callbacks_run
        
        # Skip if already done to avoid duplicate processing
        if exit_processing_done or shutdown_callbacks_run:
            logger.info("Exit processing already done or shutdown callbacks already run, skipping")
            return
            
        logger.info("Process exit detected, running end-of-session processing...")
        
        # Get active agents
        disc_agent = active_agents.get('disc')
        coach_agent = active_agents.get('coach')
        ...
        # Run processing synchronously at exit
        if disc_agent:
            try:
                logger.info("Running DISC agent end-of-session processing at exit")
                loop.run_until_complete(disc_agent.process_disc_summary())
            except Exception as e:
                logger.error(f"Error in DISC exit processing: {e}")
        
        if coach_agent:
            try:
                logger.info("Running coach agent end-of-session processing at exit")
                loop.run_until_complete(coach_agent.process_conversation())
            except Exception as e:
                logger.error(f"Error in coach exit processing: {e}")
        
        logger.info("Exit processing complete")
        exit_processing_done = True
    
    # Register the handler
    atexit.register(exit_handler)
    
    # Return the handler function for direct invocation
    return exit_handler

# Function to manually trigger exit processing
async def trigger_exit_processing():
    """Manually trigger the exit processing when room disconnects"""
    global exit_processing_done, shutdown_callbacks_run
    
    if exit_processing_done:
        logger.info("Exit processing already done, skipping")
        return
        
    logger.info("Manually triggering exit processing...")
    
    # Get active agents
    disc_agent = active_agents.get('disc')
    coach_agent = active_agents.get('coach')
    
    # Process DISC agent if available
    if disc_agent:
        try:
            logger.info("Running DISC agent end-of-session processing")
            await disc_agent.process_disc_summary()
        except Exception as e:
            logger.error(f"Error in DISC processing: {e}")
    
    # Process coach agent if available
    if coach_agent:
        try:
            logger.info("Running coach agent end-of-session processing")
            await coach_agent.process_conversation()
        except Exception as e:
            logger.error(f"Error in coach processing: {e}")
    
    logger.info("Manual exit processing complete")
    exit_processing_done = True

# Global tracking for active agents to be used by signal handlers
active_agents = {}

def setup_signal_handlers():
    """Set up signal handlers for graceful termination"""
    logger.info("Setting up signal handlers for graceful termination")
    ...
    def handle_signal(signum, frame):
        ...
        # Run processing synchronously when signal received
        if disc_agent:
            try:
                logger.info("Running DISC agent end-of-session processing for signal")
                loop.run_until_complete(disc_agent.process_disc_summary())
            except Exception as e:
                logger.error(f"Error in DISC signal processing: {e}")
        
        if coach_agent:
            try:
                logger.info("Running coach agent end-of-session processing for signal")
                loop.run_until_complete(coach_agent.process_conversation())
            except Exception as e:
                logger.error(f"Error in coach signal processing: {e}")
        
        logger.info("Signal processing complete, continuing shutdown")
...
if __name__ == "__main__":
    ...
    cli.run_app(
        WorkerOptions(
            ...
            num_idle_processes=9  # Keep 2 processes warm for better response time
        )
    )
