                                ...
                                for kb_entry in self.knowledge_base.knowledge_map[kb_map_key]["knowledgebases"]:
                                    if kb_entry["id"] == kb_id:
                                        ...
                                        if (kb_title == doc_title or 
                                            ...
                                            logger.info(f"Found matching knowledge map entry: {document_title} matched with {kb_title}")
                                            break
...
def create_doctor_system_prompt(doctor_data: DoctorData) -> str:
    ...
    system_prompt = (
        ...
        f"\n- ONLY cite sources that are marked with [[VERIFIED_SOURCE:#]] format in your context"
        f"\n- If NO sources with [[VERIFIED_SOURCE:#]] marker are provided, you MUST respond with the following in your own words or way: 'I don't have verified information about that in my knowledge base. I recommend consulting specialist literature or clinical guidelines.'"
